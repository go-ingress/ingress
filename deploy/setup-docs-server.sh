#!/usr/bin/env bash
# Hermes 文档站云主机初始化：创建专用部署账号 + SSH 通道 + 文档目录权限。
# 仅部署 docs-site（VitePress 静态产物），不涉及 Hermes 控制器（控制器部署到 K8s）。
#
# 用法（root）：
#   sudo bash deploy/setup-docs-server.sh
# 或覆盖默认值：
#   sudo DOCS_PATH=/www/wwwroot/your-domain WWW_GROUP=www bash deploy/setup-docs-server.sh
#
# 跑完后把打印的 Secrets/Variables 填进 GitHub，push main 即触发
# .github/workflows/deploy-docs.yml 自动部署。
set -euo pipefail

DEPLOY_USER="${DEPLOY_USER:-hermes-deploy}"
WWW_GROUP="${WWW_GROUP:-www}"
DOCS_PATH="${DOCS_PATH:-/www/wwwroot/hermes.example.com}"
SERVER_PORT="${SERVER_PORT:-22}"
KEY_NAME="gh_actions_hermes"

[ "$(id -u)" -eq 0 ] || { echo "请用 root 执行： sudo bash $0" >&2; exit 1; }

# 0. 确保 www 组存在（宝塔/常见面板默认有；裸机可手动 groupadd）
getent group "$WWW_GROUP" >/dev/null 2>&1 || groupadd "$WWW_GROUP"
...
[ -f "$HOME_DIR/.ssh/$KEY_NAME" ] || sudo -u "$DEPLOY_USER" ssh-keygen -t ed25519 -C gh-actions-hermes -f "$HOME_DIR/.ssh/$KEY_NAME" -N "" >/dev/null
sudo -u "$DEPLOY_USER" bash -c "cat ~/.ssh/$KEY_NAME.pub >> ~/.ssh/authorized_keys && chmod 600 ~/.ssh/authorized_keys"

# 4. 自测 SSH 闭环
if sudo -u "$DEPLOY_USER" ssh -i "$HOME_DIR/.ssh/$KEY_NAME" -o BatchMode=yes -o StrictHostKeyChecking=no -p "$SERVER_PORT" "$DEPLOY_USER@127.0.0.1" 'echo deploy-ok' >/dev/null 2>&1; then
  echo "✓ SSH 自测通过"
else
...
echo "──────────────────────────────────────────────────"
echo "Variables（可选，默认 /，独立域名用根）："
echo "  DOCS_BASE      = /"
echo ""
echo "✅ 配置后 git push origin main 即自动部署文档站到 $DOCS_PATH"
echo "   nginx 配置：root $DOCS_PATH; index index.html;（VitePress 已生成 .html，cleanUrls=false 无需 SPA fallback）"
